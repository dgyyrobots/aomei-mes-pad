...
const initQrcode = async () => {
  ...
  if (props.tag === 'canvas') {
    ...
    const canvasRef: HTMLCanvasElement | any = await toCanvas(unref(wrapRef) as HTMLCanvasElement, unref(renderText), options)
    if (props.logo) {
  ...
  <div v-loading="loading" :class="[prefixCls, 'relative inline-block']" :style="wrapStyle">
    ...
    >
      <div class="absolute left-[50%] top-[50%] font-bold">
        <icon color="var(--el-color-primary)" icon="ep:refresh-right" :size="30" />
