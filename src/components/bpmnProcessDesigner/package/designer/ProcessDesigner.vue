    ...
    <div class="my-process-designer__header" style="z-index: 999; display: table-row-group">
      ...
      <template v-if="!$slots['control-header']">
        <el-button-group key="file-control">
          <x-button pre-icon="ep:folder-opened" title="打开文件" @click="refFile.click()" />
          <el-tooltip effect="light" placement="bottom">
            <template #content>
              <div style="color: #409eff">
                <!-- <el-button link @click="downloadProcessAsXml()">下载为XML文件</el-button> -->
                <x-text-button title="下载为XML文件" @click="downloadProcessAsXml()" />
                <br />

                <!-- <el-button link @click="downloadProcessAsSvg()">下载为SVG文件</el-button> -->
                <x-text-button title="下载为SVG文件" @click="downloadProcessAsSvg()" />
                <br />

                <!-- <el-button link @click="downloadProcessAsBpmn()">下载为BPMN文件</el-button> -->
                <x-text-button title="下载为BPMN文件" @click="downloadProcessAsBpmn()" />
              </div>
            </template>
            <x-button pre-icon="ep:download" title="下载文件" />
          </el-tooltip>
          <el-tooltip effect="light">
            <x-button pre-icon="ep:view" title="浏览" />
            <template #content>
              <!-- <el-button link @click="previewProcessXML">预览XML</el-button> -->
              <x-text-button title="预览XML" @click="previewProcessXML" />
              <br />
              <!-- <el-button link @click="previewProcessJson">预览JSON</el-button> -->
              <x-text-button title="预览JSON" @click="previewProcessJson" />
            </template>
          </el-tooltip>
          <el-tooltip v-if="props.simulation" :content="simulationStatus ? '退出模拟' : '开启模拟'" effect="light">
            <x-button pre-icon="ep:cpu" title="模拟" @click="processSimulation" />
          </el-tooltip>
        </el-button-group>
        <el-button-group key="align-control">
          <el-tooltip content="向左对齐" effect="light">
            ...
            /> -->
            <x-button class="align align-bottom" pre-icon="fa:align-left" @click="elementsAlign('left')" />
          </el-tooltip>
          <el-tooltip content="向右对齐" effect="light">
            ...
            /> -->
            <x-button class="align align-top" pre-icon="fa:align-left" @click="elementsAlign('right')" />
          </el-tooltip>
          <el-tooltip content="向上对齐" effect="light">
            ...
            /> -->
            <x-button class="align align-left" pre-icon="fa:align-left" @click="elementsAlign('top')" />
          </el-tooltip>
          <el-tooltip content="向下对齐" effect="light">
            ...
            /> -->
            <x-button class="align align-right" pre-icon="fa:align-left" @click="elementsAlign('bottom')" />
          </el-tooltip>
          <el-tooltip content="水平居中" effect="light">
            ...
            /> -->
            <!-- class="align align-center" -->
            <x-button class="align align-center" pre-icon="fa:align-left" @click="elementsAlign('center')" />
          </el-tooltip>
          <el-tooltip content="垂直居中" effect="light">
            ...
            /> -->
            <x-button class="align align-middle" pre-icon="fa:align-left" @click="elementsAlign('middle')" />
          </el-tooltip>
        </el-button-group>
        <el-button-group key="scale-control">
          <el-tooltip content="缩小视图" effect="light">
            ...
            /> -->
            <x-button :disabled="defaultZoom < 0.2" pre-icon="ep:zoom-out" @click="processZoomOut()" />
          </el-tooltip>
        ...
        <el-button-group key="stack-control">
          ...
          <el-tooltip content="恢复" effect="light">
            <!-- <el-button
              ...
              @click="processRedo()"
            /> -->
            <x-button :disabled="!recoverable" pre-icon="ep:refresh-right" @click="processRedo()" />
          </el-tooltip>
          <el-tooltip content="重新绘制" effect="light">
            <!-- <el-button icon="el-icon-refresh" @click="processRestart" /> -->
            <x-button pre-icon="ep:refresh" @click="processRestart()" />
          </el-tooltip>
        </el-button-group>
        <x-button :disabled="simulationStatus" pre-icon="ep:plus" title="保存模型" :type="props.headerButtonType" @click="processSave" />
      </template>
      <!-- 用于打开本地文件-->
      <input id="files" ref="refFile" accept=".xml, .bpmn" style="display: none" type="file" @change="importLocalFile" />
    </div>
    <div class="my-process-designer__container">
      <div id="bpmnCanvas" ref="bpmnCanvas" class="my-process-designer__canvas" style="width: 1680px; height: 800px"></div>
