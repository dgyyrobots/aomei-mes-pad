...
// 先修改全局的css样式el-color-paramary
const setSystemTheme = (color: string) => {
  setCssVar('--el-color-primary', color)
  appStore.setTheme({ elColorPrimary: color })
  const leftMenuBgColor = useCssVar('--left-menu-bg-color', document.documentElement)
  // console.log(leftMenuBgColor,'设置系统主题颜色')
  // console.log(document.documentElement,'设置系统主题颜色查看useCssvar的赋值')
...
const setMenuTheme = (color: string) => {
  ...
  const theme: Recordable = {
    ...
    leftMenuCollapseBgActiveColor: isDarkColor
      ...
      : hexToRGB(unref(primaryColor), 0.1),
    // 左侧菜单字体颜色
    leftMenuTextColor: isDarkColor ? '#bfcbd9' : '#000000B3',
    // 左侧菜单选中字体颜色
...
// 在组件挂载之前（即 beforeMount 生命周期钩子）执行，
// 它会尝试在 document.documentElement 中获取或设置名为
// -left-menu-bg-color的 CSS 变量，并将初始值设置为 #eee。
onBeforeMount(() => {
  useCssVar('--left-menu-bg-color', document.documentElement, { initialValue: '#ebf1ff' });
  setSystemTheme('#615ea8')
})
</script>
