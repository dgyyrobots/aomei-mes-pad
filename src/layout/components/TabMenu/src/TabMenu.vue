              ...
              return unref(tabRouters).map((v) => {
                ...
                return (
                  <div
                    class={[
                      `${prefixCls}__item`,
                      'text-center text-12px relative py-10px cursor-pointer',
                      {
                  ...
                  >
                    <div  class='relative' style={unref(collapse)?"padding-top:8px;padding-bottom:12px":"align-items:center; justify-content: flex-start;display: flex;"}>
                      <span style="position: absolute; top: 7px; left: 9px;">
                         <Icon icon={item?.meta?.icon} size={unref(collapse)?'14px':'14px'}></Icon>
                      </span>
                        {!unref(showTitle) ? undefined : (
                          <span class="mt-5px px-2px text-13px" style='vertical-align: bottom;margin-left: 27px; '>{t(item.meta?.title)}</span>
                        )}
