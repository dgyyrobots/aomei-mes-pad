<template>
  <div>
    <Dialog v-model="dialogVisible" :title="dialogTitle" width="800px" @close="close">
      <el-form ref="formRef" v-loading="formLoading" label-width="120px" :model="formData" :rules="formRules">
        <el-form-item label="渠道费率" label-width="180px" prop="feeRate">
          <el-input v-model="formData.feeRate" clearable placeholder="请输入渠道费率" :style="{ width: '100%' }">
            <template #append>%</template>
          </el-input>
        </el-form-item>
        <el-form-item label="公众号 APPID" label-width="180px" prop="config.appId">
          <el-input v-model="formData.config.appId" clearable placeholder="请输入公众号 APPID" :style="{ width: '100%' }" />
        </el-form-item>
        ...
        <el-form-item label="渠道状态" label-width="180px" prop="status">
          <el-radio-group v-model="formData.status">
            <el-radio v-for="dict in getDictOptions(DICT_TYPE.COMMON_STATUS)" :key="parseInt(dict.value)" :label="parseInt(dict.value)">
              {{ dict.label }}
            </el-radio>
          </el-radio-group>
        </el-form-item>
        ...
        <div v-if="formData.config.apiVersion === 'v2'">
          ...
          <el-form-item label="" label-width="180px">
            <el-upload accept=".p12" action="" :before-upload="p12FileBeforeUpload" :http-request="keyContentUpload" :limit="1">
        ...
        <div v-if="formData.config.apiVersion === 'v3'">
          ...
          <el-form-item label="apiclient_cert.pem证书" label-width="180px" prop="config.privateCertContent">
            <el-input
          ...
          <el-form-item label="" label-width="180px" prop="privateCertContentFile">
            <el-upload
