      ...
      <el-row :gutter="20">
        ...
        <el-col :span="12">
          <el-form-item label="检测项编码" prop="indexCode">
            <el-input v-model="formData.indexCode" placeholder="请输入检测项编码" :disabled="disinput"/>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row :gutter="20">
        ...
        <el-col :span="12">
          <el-form-item label="检测项类型" prop="indexType" :disabled="disinput">
...
const formRules = reactive({
  iqcId: [{ required: true, message: '检验单ID不能为空', trigger: 'blur' }],
...
/** 打开弹窗 */
const open = async (type: string, id?: number, iqcId: number) => {
  dialogVisible.value = true
  dialogTitle.value = t('action.' + type)
  formType.value = type
  resetForm()
  formData.value.iqcId = iqcId
  // 修改时，设置数据
  if (id) {
...
const SelectIndex = () => {
  index.value.open(formData.value.iqcId);
}
